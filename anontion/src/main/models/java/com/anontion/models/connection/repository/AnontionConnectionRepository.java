...
public interface AnontionConnectionRepository extends JpaRepository<AnontionConnection, Long> {
  ...
  @Query(value = "SELECT * FROM anontion_connection WHERE sip_endpoint_a = :sipEndpointA AND sip_endpoint_b = :sipEndpointB", nativeQuery = true)
  Optional<AnontionConnection> findBySipEndpointAAndSipEndpointBRelaxed(@Param("sipEndpointA") String sipEndpointA, @Param("sipEndpointB") String sipEndpointB);  
